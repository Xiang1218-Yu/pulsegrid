...
type AutomationActionError struct {
	AutomationID string
	ActionType   string
	Err          error
}

func (e AutomationActionError) Error() string {
	return fmt.Sprintf("automation %s action %s failed: %v", e.AutomationID, e.ActionType, e.Err)
}
