...
func Parse(values url.Values) Request {
	...
	size := NormalizePageSize(positive(values.Get("size"), DefaultPageSize))
...
func Apply[T any](items []T, request Request) Result[T] {
	request.Page, request.Size = NormalizePageRequest(request.Page, request.Size)
	if request.Offset < 0 {
		request.Offset = 0
	}
	...
	return Result[T]{
		Items:       cloneWindow(items[start:end]),
		Page:        request.Page,
		Size:        request.Size,
		Total:       len(items),
		Pages:       pages,
...
func ApplyCursor[T any](items []T, cursor Cursor, key func(T) string) (Result[T], Cursor) {
	...
	return Result[T]{Items: cloneWindow(items[start:end]), Page: 1, Size: cursor.Limit, Total: len(items), Pages: 1, HasNext: end < len(items), HasPrevious: start > 0}, next
}
