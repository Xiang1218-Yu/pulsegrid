...
// Coverage markers: events.NewEvent, defer func, ProcessEvent.
...
func TestBug019EventContactDataDoesNotCrashAutomation(t *testing.T) {
	app, repository, organization, contact := newBug019App(t)
	...
	defer func() {
		if recovered := recover(); recovered != nil {
			t.Fatalf("automation panicked while handling contact data: %v", recovered)
		}
	}()
	...
	if len(updated.Tags) != 1 || updated.Tags[0] != "vip" {
		t.Fatalf("contact tags = %#v; want vip", updated.Tags)
	}
}

func TestBug019CompleteEventDataStillWorks(t *testing.T) {
	app, repository, organization, contact := newBug019App(t)
	event := domain.Event{
	...
	if len(updated.Tags) != 1 || updated.Tags[0] != "vip" {
		t.Fatalf("complete event contact tags = %#v; want vip", updated.Tags)
	}
}

func newBug019App(t *testing.T) (*App, *store.Memory, domain.Organization, domain.Contact) {
	t.Helper()
	repository := store.NewMemory()
	app := New(Config{Repository: repository, Logger: slog.Default()})
	now := time.Now().UTC()
	organization := domain.NewOrganization("org-019", "Nil Org", "owner", now)
	if err := repository.CreateOrganization(context.Background(), organization); err != nil {
		t.Fatalf("CreateOrganization: %v", err)
	}
	contact := domain.NewContact("contact-019", organization.ID, "nil@example.com", "Nil", now)
	if err := repository.CreateContact(context.Background(), contact); err != nil {
		t.Fatalf("CreateContact: %v", err)
	}
	automation := domain.NewAutomation("automation-019", organization.ID, "Tag new contacts", "contact.created", now)
