...
func (a *App) publish(ctx context.Context, event domain.Event) {
	if a.config.Events != nil {
		_ = a.config.Events.Publish(context.Background(), event)
	}
	if a.config.Jobs != nil {
		_ = a.config.Jobs.Enqueue(context.Background(), jobs.Job{Type: "recalculate", Payload: map[string]any{"organization_id": event.OrganizationID}, MaxRetry: 1})
	}
